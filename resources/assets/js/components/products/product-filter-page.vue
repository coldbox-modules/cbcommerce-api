  ...
  <div>
    <div class="col-md-3">
        ...
        <div class="row">
          <div class="col-xs-12">
            <ul class="subcategory-links list-unstyled">
              <li v-for="category in activeCategory.children" :key="`subcat_${category.id}`">
                <label>
                  <input
                    type="checkbox"
      ...
      <div v-if="searchParams.condition && categoriesListArray.length" class="widget-title">
        ...
        {{searchParams.condition}} Categories
      </div>
      <div v-if="searchParams.condition && categoriesListArray.length" class="widget-block">
        ...
        <div v-else class="row">
          <div class="col-xs-12">
            <ul class="subcategory-links list-unstyled">
              <li v-for="category in categoriesListArray" :key="`subcat_${category.id}`">
                <label>
                  <input
                    type="checkbox"
                    :name="`category_${category.id}`"
...
export default {
  ...
  created() {
    ...
    if (self.searchParams.condition && self.searchParams.condition !== "New") {
      ...
      this.getCategories({
        productCondition: self.searchParams.condition
      }).then(categoriesMap => {
  ...
  methods: {
    ...
    setPagination(pagination) {
      ...
      Vue.set(
        self,
        "pageCount",
        Math.ceil(pagination.total / pagination.maxrows)
      );
    },
    ...
    categoriesFilterChange(e) {
      ...
      if (this.filterCategories.indexOf(categoryId) > -1) {
        this.filterCategories.splice(
          this.filterCategories.indexOf(categoryId),
          1
        );
      } else {
